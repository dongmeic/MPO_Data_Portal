#Author: Josh Roll 
#Date: 8/9/2019
#Subject:  Use rnoaa package to download, select, and format historic climate data for use in SARM process
#Description: 
#Notes:
#----rnoaa package requires r version 3.5 or higher
...
#--------------------------------------------------------
	...
	#Store 
	#save(Load_Stations_Info.., file ="Climate Data/NOAA_Stations.RData")
	#Load if already stores
	Load_Stations_Info.. <- assignLoad( file ="//wpdotfill09/R_VMP3_USERS/tdb069/Data/Climate/Processed/NOAA_Stations.RData")
	                                           
	#Create a data frame of station infomration
	Station_Info.. <- Load_Stations_Info..
	Station_Info.. <- Station_Info..[!(duplicated(Station_Info..$id)),]
	
	#Determine days in the year for determining climate data compeltness below
	Calendar.. <- data.frame(Date = seq(as.Date("2010-01-01"),as.Date(paste( year(Sys.Date()),"-12-31",sep="")),1))
	Calendar..$Year <- year(	Calendar..$Date)
	Calendar_Summary.. <- Calendar.. %>%group_by(Year) %>%summarize(Days_in_Year = length(Date))
	
	#Go through each station for each site and determine data completness and quality
	#################################
	#Initialize a data frame to store all downloaded anf formatted climate data
    ...
    ################################################
    #Summarize climate data and detemrine number of records including those that are non-NA
    ...
    Select_Climate_Summary.. <- filter(Climate_Data_Summary..[Climate_Data_Summary..$Count >= 365,], datatype !="SNOW")
    
    #Do snow separatley since the data is spottier, i think due to no reporting on no snow days.  
  ...
  #Lat <- Site_Coordinates..$Latitude[ Site_Coordinates..$id%in%sub_location_id]  
	...
	#Save retreived data
	save(Stored_Climate_Data..,  file ="Climate Data/NOAA_Data.RData")
	Load_Store_Climate_Data.. <- assignLoad( file ="Climate Data/NOAA_Data.RData")
	
Load_Store_Climate_Data.. <- 	Stored_Climate_Data.. 
	#Graph the weather data
	##############################
	pdf("Reports/SARM Analysis/Climate_Data_Review_Summary.pdf", height = 11, width = 11)
